...
function ants:flattenRowCol(row, col)
	return row * self.config.cols + col
end

function ants:unflatten(idx)
	return math.floor(idx / self.config.cols), idx % self.config.cols
end

function ants:adjacentRowCol(row, col, direction)
	local rowd = 0
	local cold = 0
	if direction == "N" then
		rowd = -1
	...
	elseif direction == "C" then
		-- do nothing; rowd and cold both zero
	end
...
function ants:tileInDirection(row, col, direction)
	newrow, newcol = self:adjacentRowCol(row, col, direction)
...
function ants:myHills()
	local result = {}
	for _, hill in ipairs(self.hills) do
		if hill.owner == 0 then
			table.insert(result, hill)
		end
	end
	return result
end

function ants:enemyHills()
	local result = {}
	for _, hill in ipairs(self.hills) do
		if hill.owner ~= 0 then
			table.insert(result, hill)
